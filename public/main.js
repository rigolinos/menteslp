...
window.initScrollytelling = function () {
    ...
    function drawEcosystem(p) {
        const cx = W / 2;
        const cy = H * 0.48; // Geometric center adjusted slightly up
        const grow = smooth(0, .7, p);
        if (grow <= 0) return;

        // Initialize state object if not exists
        window.__ecoBrain = window.__ecoBrain || { 
            breathScale: 1, 
            glowIntensity: 1, 
            rotateAngle: 0, 
            explodeScale: 1, 
        ...
        const state = window.__ecoBrain;

        // Breathing loop simulation (GSAP can override these if active)
        const breath = state.breathScale !== 1 ? state.breathScale : 1 + Math.sin(time * 2) * 0.04;
        const glow = state.glowIntensity;
        
        // 1. Ambient glow
        const glowR = Math.min(W, H) * 0.4 * grow * breath * glow;
        const ambient = ctx.createRadialGradient(cx, cy, 0, cx, cy, glowR);
        ambient.addColorStop(0, `rgba(201,168,76,${0.25 * grow * glow})`);
        ambient.addColorStop(0.5, `rgba(107,63,160,${0.1 * grow * glow})`);
        ambient.addColorStop(1, 'rgba(0,0,0,0)');
        ctx.fillStyle = ambient;
        ctx.beginPath(); ctx.arc(cx, cy, glowR, 0, Math.PI * 2); ctx.fill();

        // 2. Inner core halo
        const innerR = Math.min(W, H) * 0.15 * grow * breath * glow;
        const coreHalo = ctx.createRadialGradient(cx, cy, 0, cx, cy, innerR);
        coreHalo.addColorStop(0, `rgba(255,255,255,${0.4 * grow})`);
        ...
        // 3. Orbital dust ring
        const dustCount = 20;
        const dustRadius = Math.min(W, H) * 0.25 * grow * breath * state.explodeScale;
        ctx.fillStyle = `rgba(201,168,76,${0.6 * grow})`;
        for(let i=0; i<dustCount; i++) {
            const angle = (i / dustCount) * Math.PI * 2 + time * 0.2;
            const px = cx + Math.cos(angle) * dustRadius * (0.9 + Math.sin(time + i)*0.1);
            const py = cy + Math.sin(angle) * dustRadius * (0.9 + Math.sin(time + i)*0.1);
            ctx.beginPath(); ctx.arc(px, py, 1.5, 0, Math.PI * 2); ctx.fill();
        }

        // Conexão Neural (Efeito de Hover dos Portais)
        if (window.__ecoLine && window.__ecoLine.active) {
            const { startProgress, endProgress, px, py } = window.__ecoLine;
            if (endProgress > startProgress) {
                const dx = px - cx, dy = py - cy;
                const perpX = -dy * 0.15, perpY = dx * 0.15;
                
                const p0x = cx, p0y = cy;
                const p1x = cx + dx * 0.25 + perpX, p1y = cy + dy * 0.25 + perpY;
                const p2x = cx + dx * 0.75 - perpX * 0.5, p2y = cy + dy * 0.75 - perpY * 0.5;
                const p3x = px, p3y = py;

                ctx.beginPath();
                let first = true;
                const steps = 30; // Resolução da curva
                for (let i = 0; i <= steps; i++) {
                    const t = startProgress + (i / steps) * (endProgress - startProgress);
                    if (t > 1) break;
                    const u = 1 - t;
        ...
        // 4. O Cérebro (Ampliado conforme pedido)
        ...
        if (brainImage && brainImage.complete) {
            ...
            ctx.globalAlpha = grow * (1 - state.explodeAlpha);
        ...
        */
        ...
        ctx.strokeStyle = `rgba(201,168,76,${0.15 * grow})`;
        ctx.lineWidth = 1;
        ctx.beginPath(); ctx.arc(cx, cy, strokeRadius, 0, Math.PI * 2); ctx.stroke();

        // Explosion wave
        if (state.explodeAlpha > 0) {
            const waveRadius = Math.min(W, H) * 0.4 * state.explodeScale;
            ctx.strokeStyle = `rgba(255,255,255,${state.explodeAlpha})`;
            ctx.lineWidth = 4 * state.explodeAlpha;
